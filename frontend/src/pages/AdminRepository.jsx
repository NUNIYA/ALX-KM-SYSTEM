...
const AdminRepository = () => {
  ...
  const handleStatus = async (id, status) => {
    try { await axios.patch(`http://localhost:5000/api/resources/${id}`, { status }); fetchResources(); }
    catch (err) { console.error(err); }
  };
  ...
  const handleVisibility = async (id, visibility) => {
    try { await axios.patch(`http://localhost:5000/api/resources/${id}`, { visibility }); setVisMenuId(null); fetchResources(); }
    catch (err) { console.error(err); }
  };

  const handleFlag = async (id, currentFlag) => {
     try { await axios.patch(`http://localhost:5000/api/resources/${id}`, { flagged: !currentFlag }); fetchResources(); }
     catch (err) { console.error(err); }
  };
                    ...
                    <motion.div key={res._id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                      ...
                      <div className="mt-auto grid grid-cols-2 relative z-10 shrink-0">
                         {isPending ? (
                           ...
                           <>
                              <a href={`http://localhost:5000/${res.fileUrl}`} target="_blank" rel="noreferrer" className={`py-7 text-[10px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-3 transition-all ${isDark ? 'bg-[#15171e] text-white border-r border-white/5 hover:bg-primary' : 'bg-slate-100 text-black border-r border-slate-200 hover:bg-black hover:text-white'}`}>
