...
const DocumentView = () => {
  ...
  const fetchComments = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/comments?targetType=resource&targetId=${id}`);
  ...
  const handleLike = async () => {
    try {
      const res = await axios.patch(`http://localhost:5000/api/resources/${id}/like`);
      setDoc(res.data);
    } catch (err) { console.error(err); }
  };
  ...
  const handleBookmark = async () => {
    try {
      const res = await axios.patch(`http://localhost:5000/api/resources/${id}/bookmark`);
      setDoc(res.data);
    } catch (err) { console.error(err); }
  };
  ...
  const handleMarkBestAnswer = async (commentId) => {
    try {
      await axios.patch(`http://localhost:5000/api/comments/${commentId}/best-answer`, {
      ...
      <section className={`pt-36 pb-16 px-12 border-b ${isDark ? 'bg-[#0A0A0B] border-white/5' : 'bg-white border-slate-100'}`}>
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="flex flex-wrap gap-4 mt-10">
            ...
            {doc.fileUrl && (
              <a href={`http://localhost:5000/${doc.fileUrl}`} download className={`flex items-center px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${isDark ? 'border-white/10 text-white hover:border-emerald-500 hover:text-emerald-500' : 'border-slate-200 text-black hover:border-emerald-500 hover:text-emerald-500'}`}>
